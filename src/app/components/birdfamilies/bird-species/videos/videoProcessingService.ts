    ...
    public generateThumbnail(videoFile: Blob): Promise<string> {
        ...
        return new Promise<string>((resolve, reject) => {
            ...
            video.addEventListener('canplay', event => {
                if(video.videoWidth > this.thumbnailMaxWidth){
                    let percent:number = this.thumbnailMaxWidth/video.videoWidth;
                    canvas.width = Math.floor(video.videoWidth*percent);
                    canvas.height = Math.floor(video.videoHeight*percent);
                } else {
